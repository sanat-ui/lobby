...
export async function requestToJoin(postId, userId) {
  ...
  if (!error) {
    // Get the post and requester info to create notification
    const { data: post } = await supabase
      .from('lfg_posts')
      .select('user_id, game, profiles(display_name, username)')
      .eq('id', postId)
      .single()
    ...
    if (post && requester) {
      await createNotification(
        ...
        'join_request',
        'New Join Request',
        `${requester.display_name || requester.username} wants to join your ${post.game} session`,
        { post_id: postId, requester_id: userId }
      )
    }
  }
...
export async function acceptJoinRequest(postId, requesterId) {
  const { error } = await supabase
  ...
  if (!error) {
    // Notify the requester they got accepted
    const { data: post } = await supabase
      .from('lfg_posts')
      .select('game')
      .eq('id', postId)
    ...
    if (post) {
      await createNotification(
        ...
        'request_accepted',
        'Request Accepted!',
        `Your request to join a ${post.game} session was accepted. GG!`,
        { post_id: postId }
      )
    }
  }
...
export async function declineJoinRequest(postId, requesterId) {
  const { error } = await supabase
    .rpc('decline_join_request', {
      post_id: postId,
      requester_id: requesterId
    })

  if (!error) {
    // Notify the requester they got declined
    const { data: post } = await supabase
      .from('lfg_posts')
      .select('game')
      .eq('id', postId)
    ...
    if (post) {
      await createNotification(
        ...
        'request_declined',
        'Request Declined',
        `Your request to join a ${post.game} session was not accepted this time.`,
        { post_id: postId }
      )
    }
  }
